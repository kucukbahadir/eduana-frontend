...
const EvaluationTab = React.memo(({ students, sessionId, initialEvaluations = {}, attendance = {} }) => {
  ...
  const presentStudents = useMemo(() => {
    return students.filter((student) => attendance[student.id]?.present);
  }, [students, attendance]);

  const postEvaluations = async (evaluations) => {
    // Only send evaluations for present students
    const evaluationsArray = Object.entries(evaluations)
      .filter(([userId]) => attendance[userId]?.present)
      .map(([userId, criteria]) => ({
        userId,
        ...criteria,
      }));
  ...
  const evaluationMutation = useMutation({
    ...
    onMutate: async (newEvaluations) => {
      ...
      queryClient.setQueryData(["session", sessionId], (oldSession) => {
        ...
        // Only update evaluations for present students
        const updatedSessionEvaluations = Object.entries(newEvaluations)
          .filter(([userId]) => attendance[userId]?.present)
          .map(([userId, criteria]) => ({
            user_id: userId,
  ...
  const debouncedEvaluationUpdate = useCallback(
    (evaluationData) => {
  ...
  const handleEvaluationChange = useCallback(
    (studentId, criteriaId, value) => {
      // Only allow changes for present students
      if (!attendance[studentId]?.present) {
        toast.info("Cannot evaluate students who are not present");
        return;
      }
    ...
    [evaluations, attendance, debouncedEvaluationUpdate]
  );
      ...
      <div className="flex flex-col gap-0 p-1 rounded-lg bg-muted">
        <div className="flex flex-col items-center justify-center h-96">
          <div className="text-center">
            <h1>404</h1>
            <p className="mt-2">No Students Present</p>
          </div>
          <p className="text-sm mt-4 text-muted-foreground">Tip: Mark students as present in the Attendance tab to evaluate them.</p>
        </div>
      </div>
    );
  }
  ...
  return (
    <div className={`flex flex-col gap-0 p-1 rounded-lg transition-colors duration-300 ${getBackgroundColor()}`}>
      <EvaluationTable students={presentStudents} evaluations={evaluations} handleEvaluationChange={handleEvaluationChange} />

      <div className={`relative overflow-hidden transition-all duration-300 ease-out ${hasActiveStatus ? "h-7 opacity-100" : "h-0 opacity-0"}`}>
        <div
          className={`absolute pt-1 inset-0 transform transition-all duration-300 ease-out ${
            hasActiveStatus ? "translate-y-0 opacity-100" : "-translate-y-full opacity-0"
          }`}
        >
